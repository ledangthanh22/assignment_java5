...
<div class="container-xl">
    <div class="col-10 offset-1">
        <div class="border " style="padding: 20px 20px 50px 20px; background-color: #fff;margin-top: 20px">
            <form:form action="/san-pham-chi-tiet/update" method="post" modelAttribute="detailProduct">
                <h4 class="text-center text-danger mb-3">Update Detail Product</h4>
                        ...
                        <div class="col-6">
                            ...
                            <form:select path="sanPham" cssClass="form-select">
                                <c:forEach items="${listProducts}" var="sp">
                                    <option ${detailProduct.sanPham.id == sp.id?"selected":""} value="${sp.id}">${sp.name}</option>
                                </c:forEach>
                            </form:select>
                        </div>
                        <div class="col-6">
                            ...
                            <form:select path="loaiSanPham" cssClass="form-select">
                                <c:forEach items="${listCategories}" var="lsp">
                                    <option ${detailProduct.loaiSanPham.id == lsp.id?"selected":""} value="${lsp.id}">${lsp.name}</option>
                                </c:forEach>
                            </form:select>
                        </div>
                    </div>
                        ...
                        <div class="col-6">
                            ...
                            <form:select path="mauSac" cssClass="form-select">
                                <c:forEach items="${listColors}" var="ms">
                                    <option ${detailProduct.mauSac.id == ms.id?"selected":""} value="${ms.id}">${ms.name}</option>
                                </c:forEach>
                            </form:select>
                        </div>
                        <div class="col-6">
                            ...
                            <form:select path="thuongHieu" cssClass="form-select">
                                <c:forEach items="${listPositions}" var="th">
                                    <option ${detailProduct.thuongHieu.id == th.id?"selected":""} value="${th.id}">${th.name}</option>
                                </c:forEach>
                            </form:select>
                        </div>
                    </div>
